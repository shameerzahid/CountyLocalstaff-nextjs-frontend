"use client"
import { Table, Thead, Tbody, Tr, Th, Td, Button, Flex, Text, useToken, useToast, ChakraProvider, extendTheme, useDisclosure } from "@chakra-ui/react";
...
export default function AdminUpcomingGoal() {
  ...
  const GetCurrentGoals = async () => {
    try {
      ...
      if (stats === 200) {
        dispatch(setUpcomingGoals(data));
        console.log("success")
      } else {
  ...
  useEffect(() => {
    GetCurrentGoals()
    // setUsers(upcomingGoals.users)
  }, [goalCreated])
  const upcomingGoals = useSelector((state) => state.adminUpcomingGoal.adminUpcomingGoal)
  console.log(upcomingGoals)
  const EndGoal = async () => {
    try {
    ...
    <>
        <div style={{ padding: "0 2.5rem", marginTop: "1rem" }}>
          ...
          <Flex flexDirection="row" justifyContent="flex-end" alignItems="center" marginTop="-35px" marginRight="-45px">
            <Button fontSize="15px" fontWeight="400" onClick={onOpen} borderRadius="0.25rem" height="38px" width="90px" border="1px solid #03AF9F" marginRight="10px" bg='transparent' color="#03AF9F" _hover={{ bg: '#03AF9F', color: "white" }}>
              Edit Goal
            </Button>
            ...
            <div style={{ overflow: "hidden", height: "59vh", width: "66.5%", backgroundColor: "white", borderRadius: "20px", margin: "0 0 0 0", paddingLeft: "20px", border: "1px solid #ccc", paddingRight: "20px" }}>
              ...
              <div className="tablecontainer" style={{ height: "50vh", overflowY: "auto", backgroundColor: "white" }}>
                ...
                <ChakraProvider theme={customTheme}>
                  ...
                  <EditGoal isOpen={isOpen} onClose={onClose} current={false} />
                </ChakraProvider>
